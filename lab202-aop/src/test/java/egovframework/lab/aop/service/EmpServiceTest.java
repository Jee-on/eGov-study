...
public class EmpServiceTest {
	...
	// annotation bean 인 경우
	//	@Resource(name="annotationEmpService")
	//	EmpService empService;
	
	public EmpVO makeVO() {
		// DAO 확인 - static 하게 관리하는 100개 기본 데이터 있음
		return makeVO(101);
	}
	...
	public void testDeleteEmp() throws Exception{
		...
		try{
			...
			EmpVO resulVO = empService.selectEmp(vo);
			fail("Biz Exception 이 발생해야 합니다.");

		}catch(Exception e){
			assertNotNull(e);
			assertTrue(e instanceof BizException);
			assertEquals("에러가 발상했습니다", e.getMessage());
	...
	public void testSelectEmpList() throws Exception{
		...
		//DAO에서 Emp 데이터를 관리할 때 항상 sorted 된 상태임
		...
		assertEquals("SALSEMAN", resultList.get(0).getJob());
